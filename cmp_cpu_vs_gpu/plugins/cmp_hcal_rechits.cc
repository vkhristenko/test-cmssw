// -*- C++ -*-
//
// Package:    ComparisonPlots/cmp_hcal_rechits
// Class:      cmp_hcal_rechits
//
/**\class cmp_hcal_rechits cmp_hcal_rechits.cc ComparisonPlots/cmp_hcal_rechits/plugins/cmp_hcal_rechits.cc
...
class cmp_hcal_rechits : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit cmp_hcal_rechits(const edm::ParameterSet&);
  ~cmp_hcal_rechits();
...
private:
  ...
  comparator<100, 0, 100> cmp_energy;
  comparator<100, -10, 10> cmp_time;
  comparator<100, -10, 10> cmp_time_falling;
  comparator<100, 0, 100> cmp_chiSquared;
  comparator<100, 0, 100> cmp_raw_energy;
  comparator<100, 0, 100> cmp_aux_energy;
...
//
cmp_hcal_rechits::cmp_hcal_rechits(const edm::ParameterSet& iConfig)
// :
...

{
  ...
  usesResource("TFileService");  
    ...
    cmp_energy.create("energy", fs);
    cmp_time.create("time", fs);
    cmp_time_falling.create("timeFalling", fs);
    cmp_chiSquared.create("chiSquared", fs);
    cmp_raw_energy.create("raw_energy", fs);
    cmp_aux_energy.create("aux_energy", fs);
...
cmp_hcal_rechits::~cmp_hcal_rechits()
{
...
void
cmp_hcal_rechits::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   using namespace edm;
   ...
   for(int i = 0; i < (int)hRecHits->size(); i++) {
     ...
     auto const& rh = (*hRecHits)[i];
     cmp_energy.v1->Fill(rh.energy());
     cmp_time.v1->Fill(rh.time());
     cmp_time_falling.v1->Fill(rh.timeFalling());
     cmp_chiSquared.v1->Fill(rh.chi2());
     cmp_raw_energy.v1->Fill(rh.eraw());
     cmp_aux_energy.v1->Fill(rh.eaux());
   ...
   for(int i = 0; i < (int)hRecHitsGPU->size(); i++) {
     ...
     auto const& rh = (*hRecHitsGPU)[i];
     cmp_energy.v2->Fill(rh.energy());
     cmp_time.v2->Fill(rh.time());
     cmp_time_falling.v2->Fill(rh.timeFalling());
     cmp_chiSquared.v2->Fill(rh.chi2());
     cmp_raw_energy.v2->Fill(rh.eraw());
     cmp_aux_energy.v2->Fill(rh.eaux());
   ...
   for (unsigned int i=0; i<hRecHits->size(); i++) {
       ...
       for (unsigned int j=0; j<hRecHitsGPU->size(); j++) {
           ...
           if (hit_cpu.id() == hit_gpu.id()) {
               ...
               cmp_energy.correlation->Fill(hit_cpu.energy(), hit_gpu.energy());
               cmp_energy.ratio->Fill(hit_cpu.energy() / hit_gpu.energy());
               
               cmp_time.correlation->Fill(hit_cpu.time(), hit_gpu.time());
               cmp_time.ratio->Fill(hit_cpu.time() / hit_gpu.time());
               
               cmp_time_falling.correlation->Fill(
                    hit_cpu.timeFalling(), hit_gpu.timeFalling());
               cmp_time_falling.ratio->Fill(
                    hit_cpu.timeFalling() / hit_gpu.timeFalling());
               
               cmp_chiSquared.correlation->Fill(
                    hit_cpu.chi2(), hit_gpu.chi2());
               cmp_chiSquared.ratio->Fill(
                    hit_cpu.chi2() / hit_gpu.chi2());
               
               cmp_raw_energy.correlation->Fill(
                    hit_cpu.eraw(), hit_gpu.eraw());
               cmp_raw_energy.ratio->Fill(
                    hit_cpu.eraw() / hit_gpu.eraw());
               
               cmp_aux_energy.correlation->Fill(
                    hit_cpu.eaux(), hit_gpu.eaux());
               cmp_aux_energy.ratio->Fill(
                    hit_cpu.eaux() / hit_gpu.eaux());
...
void
cmp_hcal_rechits::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
cmp_hcal_rechits::endJob()
{
}

// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void
cmp_hcal_rechits::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
/*
void 
cmp_hcal_rechits::ClearVariables(){
  RecHitEnergy = 0;
  depth=0;
  iEta = 0;
  iPhi = 0;
...
DEFINE_FWK_MODULE(cmp_hcal_rechits);
